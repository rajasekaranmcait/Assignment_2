package KingTwoJavaPackage;
import java.io.*;
import javax.swing.*;
import javax.swing.filechooser.FileNameExtensionFilter;
public class CsvToJsnJava extends JFrame
{
  ...
  public CsvToJsnJava()
  {
    ...
    int option = choice.showOpenDialog(this);//show the file path to choose
    if (option == JFileChooser.APPROVE_OPTION)
       CSVFile = choice.getSelectedFile();//choose the csv file from the path
    else
    {
      //To exit from the file path if don't want to choose file
  ...
  {
	CsvToJsnJava parse = new CsvToJsnJava();//create object 
    parse.convert();
    System.exit(0);
  }
  private void convert()
  {
    ...
    try
    {
      ...
      while(true)
      {
        ...
        { //if number columns equal to number entries
          write.write("{");
          for (int k = 0; k < num_cols; ++k)
          { //for each column 
            if (tokens[k].matches("^-?[0-9]*\\.?[0-9]*$"))
            { //if a number
